...
/* arama kutusuna bir kelime yazınca o kelimeyi içeren ürünleri filtrelemesi için */
  ...
  $toplamKayitSorgusu=$baglan->prepare("SELECT * FROM urunler WHERE UrunTuru='sunumluk' AND Durumu='1' $menuKosulu $aramaKosulu ORDER BY id DESC");//durumu=1 yanisilinmemiş ürünleri göster
  ...
  $tumUrunSayisi_sorgu=$baglan->prepare("SELECT SUM(UrunSayisi) AS toplamUrun FROM menuler WHERE UrunTuru='sunumluk'");
...
<div id="TamSayfaAlani">
  <div id="TamSayfaSinirlamaAlani">
    ...
    <div id="TamSayfaSolSutunSinirlamaAlani">
      <div id="TamSayfaSolSutunIcMenuAlani">
        <div id="TamSayfaSolSutunIcMenuAlaniUstMenuBaslik"><a href="index.php?sk=107">SUNUMLUKLAR</a></div>
        <div id="TamSayfaSolSutunIcMenuAltMenuBaslik">
          <img src="img/UstMenuOk.png" border="0">
          <span class="AltMenuBaslik">
            <a href="index.php?sk=107">Tüm Ürünler(<?php echo $tumUrunSayisi_sorgu["toplamUrun"]; ?>)</a>
          </span>
        </div>
        <?php
          $menuler_sorgusu=$baglan->prepare("SELECT * FROM menuler WHERE UrunTuru='sunumluk' ORDER BY MenuAdi ASC");// menu adını alfabetik sıraya göre dizcek
        ...
        <div id="MenuSecenekleriKapsayiciAlani">
          <ul id="MenuSeceneklerAlani">
            <li class="MenuSecenekleri">
              <img src="img/menuok.png" border="0"> <a href="index.php?sk=107&MenuId=<?php echo $menu["id"]; ?>"><?php echo  DonusumleriGeriDondur($menu["MenuAdi"]); ?> (<?php echo  DonusumleriGeriDondur($menu["UrunSayisi"]); ?>)</a>
            </li>
          </ul>
        </div>
        <?php
    ...
    <div id="TamSayfaSagSutunSinirlamaAlani">
      <div id="TamSayfaSagSutunAramaAlani">
        <form action="
                    <?php
                       if($menuKosulu!=""){ ?>index.php?sk=107&MenuId=<?php echo $gelenMenuId; ?>
     ...
     <div id="TamSayfaSagSutunUrunlerAlani">
        <div id="TamSayfaSagSutunUrunlerDengelemeAlani">
            <?php
              $urun_sorgu=$baglan->prepare("SELECT * FROM urunler WHERE UrunTuru='sunumluk' AND Durumu='1' $menuKosulu $aramaKosulu ORDER BY id DESC LIMIT $sayfalamayaBaslanacakKayitSayisi,$sayfaBasinaKayitSayisi");//silinen ürünün durumu 0 olcak. o yuzden 1 ola silinmemiş olnları listeliycez
            $urun_sorgu->execute();
      ...
      <div class="TamSayfaSagSutunUrunlerSinirlamaAlani">
          <div class="TamSayfaSagSutunUrunResmiDisiAlani">
            <a href="index.php?sk=83&ID=<?php echo DonusumleriGeriDondur($kayit["id"]); ?>">
                <img src="img/urunler/sunumluk/<?php echo DonusumleriGeriDondur($kayit["UrunResmiBir"]); ?>" border="0" >
            </a>
          </div>

          <div class="TamSayfaSagSutunUrunUstBaslik">
              <a href="index.php?sk=83&ID=<?php echo DonusumleriGeriDondur($kayit["id"]); ?>" >Sunumluklar</a>
          </div>
          <div class="TamSayfaSagSutunUrunUstBaslik">
  ...
  <div class="SayfalamaAlaniKapsayicisi">
    ...
    <div class="sayfalamaAlaniIciNumaraAlaniKapsayicisi">
      <?php 
        if ($sayfalama>1) {
          echo "<span class='Sayfalamapasif'><a href='index.php?sk=107' . $sayfalama_kosulu. '&SYF=1'> << </a> </span>";
          $sayfalamaIcınSayfalamaDegeriniBirGeriAl=$sayfalama-1;
          echo "<span class='Sayfalamapasif'><a href='index.php?sk=107" . $sayfalama_kosulu . "&SYF=" . $sayfalamaIcınSayfalamaDegeriniBirGeriAl . "'> < </a> </span>";
        }
        for ($i=$sayfalama-$sayfalamaSolSagButonSayisi; $i<=$sayfalama+$sayfalamaSolSagButonSayisi;$i++) { 
          if (($i>0) and ($i<=$bulunanSayfaSayisi)) {
            if ($sayfalama==$i) {
                ...
                echo "<span class='Sayfalamapasif'><a href='index.php?sk=107" .$sayfalama_kosulu."&SYF=" . $i . "'> ". $i. "</a></span>";
            }
          }
        }
        if ($sayfalama!=$bulunanSayfaSayisi) {
            $sayfalamaIcınSayfalamaDegeriniBirIleriAl=$sayfalama+1;
            echo "<span class='Sayfalamapasif'> <a href='index.php?sk=107".$sayfalama_kosulu ."&SYF=" . $sayfalamaIcınSayfalamaDegeriniBirIleriAl . "'> > </a> </span>";
            echo "<span class='Sayfalamapasif'><a href='index.php?sk=107".$sayfalama_kosulu. "&SYF=" . $bulunanSayfaSayisi . "'> >> </a></span>";
        }
    ?>
    </div>
  </div>
